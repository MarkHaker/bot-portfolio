...
# Инициализация бота и скрытой клавиатуры
bot = TeleBot(TOKEN)
hideBoard = types.ReplyKeyboardRemove() 

# Константа для кнопки отмены
...
def gen_markup(rows):
    """Генерация обычной клавиатуры с кнопкой отмены"""
...
# Атрибуты проектов для редактирования
...
def info(message):
    """Обработчик команды /info - отображает список доступных команд"""
    bot.send_message(message.chat.id,
"""
Вот команды которые могут тебе помочь:

/new_project - используй для добавления нового проекта
/skills — добавить навык к какому-либо проекту.
/projects — посмотреть текущие/существующие проекты.

/delete — удалить проект.

/update_projects — обновить имя проекта, описание, ссылку, статус.
...
def callback_project(message, data, statuses):
    """Получение статуса проекта и сохранение проекта"""
    status = message.text
...
def skill_handler(message):
    """Начало процесса добавления навыка к проекту"""
    user_id = message.from_user.id
...
def callback_query(call):
    """Обработчик inline кнопок - отображение информации о проекте"""
...
def update_project(message):
    """Начало процесса обновления информации о проекте"""
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
...
def update_project_step_3(message, project_name):
    """Выбор атрибута проекта для обновления"""
...
def update_project_step_4(message, project_name, attribute): 
    """Обновление выбранного атрибута проекта"""
...
def text_handler(message):
    """Обработчик текстовых сообщений - проверка, не является ли текст названием проекта"""
    user_id = message.from_user.id
